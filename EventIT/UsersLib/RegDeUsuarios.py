        ...
        with open(path,'r') as f:
            for linea in f.readlines():
                if linea.split('/')[0] == 'Admin':
                    ...
                    cuil = linea.split('/')[4]
                    contactosDeInteres = list(map(lambda x:self.searchCitizen(cuil=x), list(filter(lambda x:x!=',', list(linea.split('/')[5].split('[')[1].split(']')[0])))))
                    listaDeSolicitudes = list(map(lambda x:self.searchCitizen(cuil=x), list(filter(lambda x:x!=',', list(linea.split('/')[6].split('[')[1].split(']')[0])))))
                    listaDeRechazos = list(map(lambda x:self.searchCitizen(cuil=x), list(filter(lambda x:x!=',', list(linea.split('/')[7].split('[')[1].split(']')[0])))))
