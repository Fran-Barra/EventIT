...
class Estadisticas:
    ...
    def calculate_percentage_of_atendees_of_the_zone(self):
        porcentaje_por_evento =dict({})
        for evento in self.lista_de_eventos:
            if self.calculate_total_number_of_attendees()[evento] != 0:
                porcentaje_por_evento[evento] = round((self.calculate_number_of_attendees_per_zone_per_event()[evento] / self.calculate_total_number_of_attendees()[evento]) * 100, 1)
                '0' + str(porcentaje_por_evento[evento]) if porcentaje_por_evento[evento] < 10 else str(porcentaje_por_evento[evento])
                '0' + str(porcentaje_por_evento[evento]) if porcentaje_por_evento[evento] < 100 else str(porcentaje_por_evento[evento])
            else:
                porcentaje_por_evento[evento] = '000.0'
        return porcentaje_por_evento

  # porcentaje_de_asistentes_de_la_zona_por_evento
